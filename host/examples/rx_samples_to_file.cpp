...
template<typename samp_type> void recv_to_file(
    ...
    bool pingpong = false,
    unsigned long long pingpong_size = 5000000
    ){
        ...
        if (not null) {
            if (not pingpong)
                ...
                outfile.open("ping.dat", std::ofstream::binary);
                isfileA = true;
            }
            ...
            if (pingpong)
            {
                if (cnt >= pingpong_size)
                {
                    ...
                    if (isfileA)
                    {
                        ...
                        outfile.open("ping.dat", std::ofstream::binary);
                        isfileA = false;
                        //std::cout << "pingpongB.dat"<<std::endl;
                    } 
                    else
                    {
                        if (outfile.is_open())
                            outfile.close();
                        outfile.open("pong.dat", std::ofstream::binary);
                        //std::cout << "pingpongA.dat"<<std::endl;
                        isfileA = true;
                    }
                    cnt = 0;
                }
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    size_t total_num_samps, spb, pingpong_size;
    ...
    desc.add_options()
        ...
        ("pingpong","output the data into ping.dat and pong.dat as ping-pong buffer")
        ("pingpong_size",po::value<size_t>(&pingpong_size)->default_value(5000000), "size of each ping-pong file")
        ;
    po::variables_map vm;
    ...
    bool pingpong = vm.count("pingpong") > 0;
...
#define recv_to_file_args(format) \
    (usrp, format, wirefmt, file, spb, total_num_samps, total_time, bw_summary, stats, null, enable_size_map, continue_on_bad_packet,pingpong,pingpong_size)
    //recv to file
